...
// longer the binding cap: at the measured worst-case entry size (282 B — a Codex
// 64-char session key plus a memory-compress delta) 1000 sessions would be ~275 KB,
// past the read cap, which is exactly the mismatch that used to collapse the file.
...
// Validate the per-source input-delta map: { [source]: { saved } }. Each source
// key is a non-empty string; saved coerces to finite >= 0. Anything malformed
// (non-object, negative, NaN) is dropped so a tampered or partial line can never
// poison the aggregate sum. A legacy `baseline` key on an existing entry is simply
// dropped here (never read) — old ledgers stay readable, no migration needed.
...
export function recordInputDelta(delta, historyPath = getHistoryPath()) {
  ...
  entry.inputDeltas[delta.source] = { saved };
  byId.set(delta.sessionKey, entry);
  return writeEntries(byId, historyPath, delta.sessionKey);
}
