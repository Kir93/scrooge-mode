...
//
// CONTRACT DRIFT: the field names read below (Claude `message.id` + `usage.*`,
// Codex `payload.info.*token_usage.*`) — and the hook-input stdin fields the hooks
// read (`session_id`/`transcript_path`/`prompt`/`source`) — are pinned by
// exact-value canaries in tests/test_session_log.js and tests/test_schema_contract.js.
// If a host renames or drops one, refresh the matching fixture AND the "Verified
// locally" date below in the same change: the canaries then fail loudly instead of
// the parser degrading to a silent 0-token EMPTY_SUMMARY.
//
...
export function parseClaudeSession(filePath) {
  ...
  for (const line of raw.split('\n')) {
    ...
    let rec = byId.get(id);
    if (!rec) {
      rec = { input: 0, output: 0, reasoning: 0, cacheRead: 0, hasUsage: false, hasToolUse: false };
      byId.set(id, rec);
    }
    if (msg.usage) {
      ...
      rec.input = msg.usage.input_tokens || 0;
      rec.output = msg.usage.output_tokens || 0;
      rec.cacheRead = msg.usage.cache_read_input_tokens || 0;
      // Claude folds thinking into output_tokens, so a separate reasoning count is
      // usually absent (→ 0). Captured defensively when a host does surface it, so
      // the honest bill can show reasoning on its own "uncompressed" line.
      rec.reasoning = msg.usage.reasoning_output_tokens || 0;
      rec.hasUsage = true;
    }
  ...
  for (const rec of byId.values()) {
    ...
    reasoningOutputTokens += rec.reasoning;
...
// Per-turn view of a Claude transcript: the same byId dedup as
// parseClaudeSession, but exposed as a turn-index-ordered array instead of one
// summed summary. Additive — the aggregate API above is unchanged. Never
// throws; unreadable / malformed input degrades to an empty (or partial) array.
//
// Each element also carries `text`: the assistant text blocks scattered across
// same-id JSONL lines, concatenated in stream order on top of the id dedup
// (an exact-duplicate block re-emitted for a usage update is collected once).
// tool_use-only responses have text ''. turnIndex is 1-based so "turn 1" in
// drift trajectories reads literally.
export function parseClaudeSessionTurns(filePath, { isSubagent = false } = {}) {
  let raw;
  try {
    raw = fs.readFileSync(filePath, 'utf8');
  } catch (e) {
  ...
  const byId = new Map(); // insertion order = stream order = turn order
  let lineIndex = -1;
  for (const line of raw.split('\n')) {
    lineIndex++;
    if (!line.trim()) continue;
    ...
    let rec = byId.get(id);
    if (!rec) {
      rec = { id, model: null, output: 0, hasUsage: false, hasToolUse: false, texts: [] };
      byId.set(id, rec);
    }
    if (msg.usage) {
      rec.output = msg.usage.output_tokens || 0;
    ...
    if (Array.isArray(msg.content)) {
      for (const block of msg.content) {
        if (!block) continue;
        if (block.type === 'tool_use') rec.hasToolUse = true;
        else if (block.type === 'text' && typeof block.text === 'string' && block.text && !rec.texts.includes(block.text)) {
          rec.texts.push(block.text);
        }
      }
    }
  }
  ...
  const turns = [];
  for (const rec of byId.values()) {
    // Same filter as the aggregate: a record without usage is not a counted
    // response, so per-turn length always equals the aggregate `turns`.
    if (!rec.hasUsage) continue;
    turns.push({
...
// Opt-in enumeration of a session's subagent transcripts:
// <project>/<uuid>.jsonl → <project>/<uuid>/subagents/*.jsonl. A separate
// entry point on purpose — the SKIP_DIRS recency scan above keeps excluding
// subagents; callers opt in per session. Empty array when none exist.
export function listSubagentTranscripts(sessionFile) {
  try {
    const dir = path.join(
...
export function parseCodexSession(filePath) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (totalUsage) {
      ...
      seenTotals.add(sig);
      totalInputTokens = tokenNumber(totalUsage.input_tokens);
      totalOutputTokens = visibleOutputTokens(totalUsage);
      // Reasoning is subtracted from visible output above; surface it separately
      // (cumulative, so last-wins) for the honest bill's uncompressed line.
      totalReasoningTokens = tokenNumber(totalUsage.reasoning_output_tokens);
    ...
    if (!usage) continue;
    fallbackInputTokens += tokenNumber(usage.input_tokens);
    fallbackOutputTokens += visibleOutputTokens(usage);
    fallbackReasoningTokens += tokenNumber(usage.reasoning_output_tokens);
  ...
  return {
    ...
    toolUseOutputTokens: 0,
    reasoningOutputTokens: fallbackReasoningTokens,
