...
- **The JA/ZH landings carry no benchmark numbers, and must not start.** That is what keeps them honest without a mirror obligation: a landing with a savings figure silently goes stale every time a measurement moves, and nothing guards it. Keep them to value, install, activation, and one before/after example; link to the canonical README for every number.
...
- Substantive rule changes stay mirrored across `ko`/`en`/`ja`/`hi`/`zh`, or the PR explains why parity is intentionally not changed.
