...
skill-only 호스트는 register 규칙을 skill로 받지만 활성화는 수동 — 턴마다 reinject hook 없음, 토큰 stats 없음. 완전한 hook+stats+statusline은 Claude Code, Codex는 `~/.codex/config.toml`로 hook+stats 제공. Codex는 `UserPromptSubmit`만 배선(`SessionStart` 없음)하므로 업데이트 알림·`↑vX` 마커는 Claude 전용. Codex에서 업그레이드는 재설치로 처리함(INSTALL.ko.md의 [Update](INSTALL.ko.md#update) 참고).
...
`terse`보다도 짧으므로 단순 brevity가 아니라 register 효과임. `caveman:full`은 비슷한 token 수(**1005** vs **1042**)지만 문법을 깨고 그럼. 같은 held-out 코퍼스에서 두 arm을 판정하면 주장 보존이 scrooge **0.68** vs caveman **0.60** — raw token은 caveman이 근소 우위지만 차별 축은 이 fidelity 차이임. 다만 safety prose 보존은 양쪽 동일(각 12/16). 손실은 breadth이지 오정보가 아님. rows: [토큰](./benchmarks/published/results-ko-clean-opus48.jsonl) · [judge](./benchmarks/published/results-ko-fidelity.jsonl).
...
`caveman:full`은 raw token이 더 적음(**703** vs **816**). 두 arm을 함께 판정하면 scrooge가 주장을 더 보존하나(**0.72** vs **0.69**, 11개 중 9개 우세) 격차가 작고 safety 보존은 동일함(각 9/11) — [caveman fidelity](./benchmarks/README.md#caveman-fidelity--the-differentiation-measured). rows: [토큰](./benchmarks/published/results-en-clean-opus48.jsonl) · [judge](./benchmarks/published/results-en-fidelity.jsonl).
