...
- **Register-only isolation.** 하네스는 `claude --print --system-prompt <rule>`로 각 arm 실행 — Claude Code 기본 system prompt를 **대체**함. register 효과를 깔끔히 분리하지만, 실제 `/scrooge` 세션은 Claude Code 전체 system prompt 위에 register가 얹히므로 verbose 세션 대비 실측 절감은 헤드라인과 다를 수 있음. register **유지**는 실측 근거 확보: dogfood 세션 drift 분석(scrooge 활성 372개 중 conclusive 134개, 2026-07, [`benchmarks/session-evidence/`](./benchmarks/session-evidence/)) 결과 register가 멀티턴에서도 유지됨 — 후반부 prose output이 초반 대비 평탄(후반/전반 median ratio 0.85, conclusive의 77% retained). 유지 축만: 실세션 _절감_은 여전히 측정 불가(verbose counterfactual 부재, ADR-003). 전체 caveat는 [`benchmarks/README.md`](./benchmarks/README.md).
