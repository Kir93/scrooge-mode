...
# when its sessionId matches THIS statusline's session (read from the stdin
# payload Claude Code provides) — so a different or fresh session never shows a
# stale number. Reading a pre-rendered file avoids shelling out to node per
# keystroke. Same symlink/control-byte hardening as the state file.
